## Questão 5 - Implemente uma função que realiza uma travessia inordem (esquerda-raiz-direita) 
...
class BinaryTree:
    ...
    def in_order_recursive(self, no):
        if no is not None:
            self.in_order_recursive(no.left)
            print(no.value, end=" ")
            self.in_order_recursive(no.right)
